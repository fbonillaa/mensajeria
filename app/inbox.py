...
def send():
    if request.method == 'POST':        
        ...
        if not to_username:
            flash('No se puede enviar un mensaje sin el nombre de usuario del destinatario')
            return render_template('inbox/send.html')
        
        if not subject:
            flash('El campo del sujeto o proposito del mensaje debe estar diligenciado')
            return render_template('inbox/send.html')
        
        if not body:
            flash('El campo del contenido del mensaje no puede estar vacio')
            return render_template('inbox/send.html')    
        ...
        if userto is None:
            error = 'no se pudo entregar el mensaje, parece que el destinatario no existe'
            #el destinatario o correo destino lo llaman recipient
